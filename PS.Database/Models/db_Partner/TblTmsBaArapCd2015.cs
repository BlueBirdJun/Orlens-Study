using System;
using System.Collections.Generic;

namespace PS.Database.Models.db_Partner
{
    public partial class TblTmsBaArapCd2015
    {
        public decimal ArapCd { get; set; }
        public string UseYn { get; set; }
        public string ArapGb { get; set; }
        public string ArapCdPrt { get; set; }
        public string ArapNm { get; set; }
        public string CashFlow { get; set; }
        public decimal? ArapCdUp { get; set; }
        public string GrpYn { get; set; }
        public string AcctUnitCd { get; set; }
        public string ArapAttach { get; set; }
        public string ArapAttachCd { get; set; }
        public string AccCd { get; set; }
        public decimal? AccCdRmkSeq { get; set; }
        public string BgtItemCd { get; set; }
        public string Gbcd1 { get; set; }
        public string Gbcd2 { get; set; }
        public string Gbcd3 { get; set; }
        public string Gbcd4 { get; set; }
        public decimal? DispSeq { get; set; }
        public string ErpCd { get; set; }
        public string ModUser { get; set; }
        public string ModDt { get; set; }
        public string Bigo { get; set; }
        public string DelYn { get; set; }
        public DateTime? BitchkDt { get; set; }
        public string ArapType { get; set; }
        public string AccCdOld { get; set; }
    }
}
